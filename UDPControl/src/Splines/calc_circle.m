...
%    end
   if i == 1
       ...
       x_tmp = support(end, 1) + step_length*cos(support(end,3)+pi/2);
...
%    end
   ...
   yaw = atan((working_spline(2,idx+1)-working_spline(2,idx))/(working_spline(1,idx+1)-working_spline(1,idx)))-pi/2;
   i
...
for i = 1:length(support)
   support(i,1) = support(i,1)+sin(support(i,3)+pi/2)*0.1;
   support(i,2) = support(i,2)-cos(support(i,3)+pi/2)*0.1;
end
plot(xg,yg,'kx')
grid on
grid minor
