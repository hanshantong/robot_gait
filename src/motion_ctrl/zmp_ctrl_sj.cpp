...
void Robot::set_parameters(){
    ...
    parameters.zmp_offset_x            = 0.0;  // Offset in x direction for the ZMP. Use this variable to compensate the LIPM and MBM model error (ca 2-4 cm depending on walking speed)
    parameters.zmp_offset_y            = 0.047;  // Offset in x direction for the ZMP. Use this variable to compensate the LIPM and MBM model error (ca 2-4 cm depending on walking speed)
    ...
    parameters.Tipping_Scale_right     = 0.8; //88    //* This value modifies the hip roll in case of soft joints. Due to control schemes, this value can be kept at 1. 
...
/*void Robot::stopGait(){
    ...
    if(leftIsSupportLeg == 1){
        if((walking_state == 3) || (walking_state == 4) || (walking_state == 6) || (walking_state == 7))
        {
            if(pointsTillDsp < step_duration*(100-parameters.dsp_percentage)/100 )
            //stop in ssp, move to next footprint
            {
            ...
            //stop in dsp, stay in this footprint
        ...
        {
            if(pointsTillDsp < step_duration*(100-parameters.dsp_percentage)/100
                && ((walking_state == 3) || (walking_state == 4) || (walking_state == 6) || (walking_state == 7)))
            {
                // std::cout << "in ssp" <<std::endl;
                        y_zmp_start = y_support_new +parameters.zmp_offset_y*cos(psi_support_new);
...
/*void Robot::stopGait(){
    ...
    int new_time_step;
    int stop_in_first_double=0;
    if(time_step > step_duration*parameters.dsp_percentage/100)
    {
        new_time_step = time_step - parameters.dsp_percentage/100*step_duration;
        pointsTillDsp = step_duration - new_time_step % step_duration;
    }
    else
    {
        stop_in_first_double = 1;
        pointsTillDsp = step_duration*parameters.dsp_percentage/100 - time_step;
    } 
...
/*void Robot::stopGait(){
    ...
    int new_time_step;
    int stop_in_first_double=0;
    if(time_step > parameters.dsp_percentage/100*step_duration)
    {
        new_time_step = time_step - parameters.dsp_percentage/100*step_duration;
        pointsTillDsp = step_duration - new_time_step % step_duration;
    }
    ...
    {
        stop_in_first_double = 1;
        pointsTillDsp = parameters.dsp_percentage/100*step_duration - time_step;
    } 
...
void Robot::calc_p_ref_heel_circling(int clockwise, double radius, int step_amount, double step_psi)
{
    ...
    radius = parameters.robot_width/2;
    ...
    for (int i = 0; i < step_amount; i++){  //bug fixed.
        ...
        sy[i] =  invert_y * radius * (1-cos(so[i]));
        sx[i] =  radius * sin(so[i]);
        printf("%f %f %f\n",so[i],sx[i],sy[i]);
    }
...
void Robot::calc_p_ref_heel_curve(int forward, int leftFootStart, std::vector<double> x_footprint_on_spline, std::vector<double> y_footprint_on_spline, std::vector<double> yaw_footprint_on_spline){
    ...
    ref_x_support_foot_trajectory.insert(ref_x_support_foot_trajectory.end(), ceil(step_duration*parameters.dsp_percentage/100), 0);
    if(leftFootStart == 0){
        ref_y_support_foot_trajectory.insert(ref_y_support_foot_trajectory.end(), ceil(step_duration*parameters.dsp_percentage/100), -parameters.robot_width/2);
    }
    else{
        ref_y_support_foot_trajectory.insert(ref_y_support_foot_trajectory.end(), ceil(step_duration*parameters.dsp_percentage/100), parameters.robot_width/2);
    }
    ref_psi_support_foot_trajectory.insert(ref_psi_support_foot_trajectory.end(), ceil(step_duration*parameters.dsp_percentage/100), 0);
    ...
    ref_x_support_foot_trajectory.insert(ref_x_support_foot_trajectory.end(), floor(step_duration*(1-parameters.dsp_percentage/100)), 
        ref_x_support_foot_trajectory.back());
    ref_y_support_foot_trajectory.insert(ref_y_support_foot_trajectory.end(), floor(step_duration*(1-parameters.dsp_percentage/100)), ref_y_support_foot_trajectory.back());
    ref_psi_support_foot_trajectory.insert(ref_psi_support_foot_trajectory.end(), floor(step_duration*(1-parameters.dsp_percentage/100)),ref_psi_support_foot_trajectory.back());
...
void Robot::calc_p_ref_circling( std::vector<double> *p_ref_x, std::vector<double> *p_ref_y, int clockwise, 
    ...
    // Last Steps
    p_ref_x->insert(p_ref_x->end(), step_duration * (1 - parameters.dsp_percentage/100), sx[step_amount-1] + (1) * sin(so[step_amount-1]) * ( parameters.robot_width/2));
...
void Robot::calc_p_ref_transition_circling(int clockwise, double radius, int step_amount, double step_psi)
{
    ...
    for (int i = 0; i < step_amount - 1; i++){
        if(i == 0){
            ...
            t_psi[0]    =   0.0 * ssp_duration;//dsp_duration;
            t_psi[1]    =   0.01* ssp_duration;//dsp_duration;;
            t_psi[2]    =   0.2 * ssp_duration;//dsp_duration;;
            t_psi[3]    =   0.6 * ssp_duration;//dsp_duration;;
            t_psi[4]    =   0.99* ssp_duration;//dsp_duration;;
            t_psi[5]    =   1.0 * ssp_duration;//dsp_duration;;
            ...
            for (int k = 0; k < dsp_duration; k++){
                ...
                ref_psi_body.push_back(psi1);
                //ref_psi_body.push_back(psi_increment);
            }
            


            for(int k=0; k < ssp_duration; k++)
            {
                ref_x_zmp_trajectory.push_back(x2);
                ref_y_zmp_trajectory.push_back(y2);
    ...
    std::vector<double> * theta_ref_swing_foot_complete, std::vector<double> * psi_ref_swing_foot_complete)
{
    ...
    t_posx[5] = 0.4     *cycle_duration;
    t_posx[6] = 0.5     *cycle_duration;
    t_posx[7] = 0.6     *cycle_duration;
...
void Robot::calculate_swing_foot_piece_trajectory_heel_curve( int flag, 
    ...
    std::vector<double> t_posz(9), t_posx(14),t_posy(14), t_theta(7), theta(7), x(14), z(9), y(14);
    std::vector<double> t_psi(5),psi(5);
    // Calculate x
    double step_length = x_swing_foot_end-x_swing_foot_start;
    double t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,zk0,zk1,zk2,zk3,zk4,zk5,zk6, zk7,zk8;
    ...
    t_psi[2] = t5 * cycle_duration;
    t_psi[3] = t7 * cycle_duration;
    t_psi[4] = t8 * cycle_duration;

    psi[0] = psi_start + 0.00 * delta_psi;
    psi[1] = psi_start + 0.00 * delta_psi;;
    psi[2] = psi_start + 0.5 * delta_psi;;
    psi[3] = psi_start + 1.00 * delta_psi;
    psi[4] = psi_start + 1.00 * delta_psi;       
...
void Robot::calculate_swing_foot_complete_trajectory_heel_curve(std::vector<double> x_footprint_on_spline, std::vector<double> y_footprint_on_spline, std::vector<double> yaw_footprint_on_spline,
    ...
    ref_x_swing_foot_trajectory.insert(ref_x_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), 0);
    if(leftFootStart == 0){
        ref_y_swing_foot_trajectory.insert(ref_y_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), parameters.robot_width/2);
    }
    else{
        ref_y_swing_foot_trajectory.insert(ref_y_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), -parameters.robot_width/2);
    }
    ref_z_swing_foot_trajectory.insert(ref_z_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), parameters.robot_ankle_to_foot);

    ref_psi_swing_foot_trajectory.insert(ref_psi_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), 0);
    ref_theta_swing_foot_trajectory.insert(ref_theta_swing_foot_trajectory.end(), ceil(step_duration * parameters.dsp_percentage/100), 0);
...
/*******************************Kicking Part START*******************************/
        ...
        void Robot::kick(bool isLeftLeg, int kickType, double pitch_angle) {
            walking_state = 8;
            double target_x = 0.2;
            ...
            int size=0;
            kick_calc_swing(isLeftLeg, kickType, target_x, target_y, target_z, yaw_angle, pitch_angle);
...
//Kicking Swing Leg Calculation, created by rpf
        void Robot::kick_calc_swing(bool isLeftLeg, int kickType, double target_x,double target_y, double target_z, double yaw_angle, double pitch_angle) {
...
//    const int pointNum = 7;
    ...
    //double kick2lift_time;
        ...
        //lift to kick
                Robot::kick_swing_trajectory_generator(LIFT_X_BACK, parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, target_x, target_y, target_z, 0, 0, kick_time,1);
        //kick to lift
        // Robot::kick_swing_trajectory_generator(target_x, target_y, target_z, 0, 0, 0, parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, kick2lift_time,2);
        //lift to init
        // Robot::kick_swing_trajectory_generator(0, parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, 0, parameters.robot_width/2, parameters.robot_ankle_to_foot, 0, 0, KICK_LIFT2INIT_TIME,3);
        //kick to dsp
                Robot::kick_swing_trajectory_generator(target_x, target_y, target_z, 0, 0, DSP_KICKLEG_X, parameters.robot_width/2, parameters.robot_ankle_to_foot, 0, 0, kick2dsp_time, 2); 
            }
            else {
        //init to lift
        ...
        //lift to kick
                Robot::kick_swing_trajectory_generator(0, -parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, target_x, target_y, target_z, 0, 0, KICK_SWING_TIME_STRONG,1);
        //kick to lift
        // Robot::kick_swing_trajectory_generator(target_x, target_y, target_z, 0, 0, 0, -parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, kick2lift_time,2);
        //lift to init
        // Robot::kick_swing_trajectory_generator(0, -parameters.robot_width/2, parameters.robot_ankle_to_foot + LIFT_HEIGHT, 0, 0, 0, -parameters.robot_width/2, parameters.robot_ankle_to_foot, 0, 0, KICK_LIFT2INIT_TIME,3);
...
//Kick Swing Trajectory Generator, created by liuyuezhang 2017-7-16
        void Robot::kick_swing_trajectory_generator(double startX, double startY, double startZ, double startTheta, double startPsi, 
            ...
            double offsetTheta = endTheta - startTheta;
            double offsetPsi = endPsi - startPsi;
...
//Kicking Support Leg Calculation, created by lhy
    ...
    //init to dsp
                                                ...
                                                if(isLeftLeg){
                                                    ...
                                                    ref_y_support_foot_trajectory.insert(ref_y_support_foot_trajectory.end(),ceil((KICK_INIT_TIME + KICK_COMTRANS_TIME + KICK_LIFT_TIME+ kick_time + kick2dsp_time +  KICK_DSP_TIME)/parameters.Ts),-(parameters.robot_width/2));
                                                }
                                                else{
                                                    ref_x_support_foot_trajectory.insert(ref_x_support_foot_trajectory.end(),ceil((KICK_INIT_TIME + KICK_COMTRANS_TIME + KICK_LIFT_TIME+ kick_time + kick2dsp_time +  KICK_DSP_TIME)/parameters.Ts), 0);
                                                    ref_y_support_foot_trajectory.insert(ref_y_support_foot_trajectory.end(),ceil((KICK_INIT_TIME + KICK_COMTRANS_TIME + KICK_LIFT_TIME+ kick_time + kick2dsp_time +  KICK_DSP_TIME)/parameters.Ts),parameters.robot_width/2);
                                                ...
                                                ref_z_support_foot_trajectory.insert(ref_z_support_foot_trajectory.end(),(KICK_INIT_TIME + KICK_COMTRANS_TIME + KICK_LIFT_TIME+ kick_time + kick2dsp_time +  KICK_DSP_TIME)/parameters.Ts,parameters.robot_ankle_to_foot);
                                                ref_psi_support_foot_trajectory.insert(ref_psi_support_foot_trajectory.end(),(KICK_INIT_TIME + KICK_COMTRANS_TIME + KICK_LIFT_TIME+ kick_time + kick2dsp_time +  KICK_DSP_TIME)/parameters.Ts,0);
...
//    //start kicking
    ref_x_zmp_trajectory.insert(ref_x_zmp_trajectory.end(), kick_time/parameters.Ts, 0);
...
//     // std::cout << "length: " << x_ref.size() << std::endl;
//     if(time_step > 0 && time_step < parameters.N){
//         return x_ref.at(time_step);
//     }
//     else{
//         return 0;
//     }
...
// float Robot::get_psi(){
//     if(time_step > 0 && time_step < parameters.N){
//         return ref_psi_body.at(time_step);
//     }
//     else{
//         return 0;
//     }
