...
void Robot::set_parameters(){
    ...
    parameters.step_duration            = 1;     //* Duration for one step. This step consists of 80% SSP and 20% DSP. NOT gait cycle duration (which is 2*step_duration)
    ...
    parameters.zmp_offset_y             = -0.03;  // Offset in x direction for the ZMP. Use this variable to compensate the LIPM and MBM model error (ca 2-4 cm depending on walking speed)
    ...
    parameters.dsp_percentage      = 5.0;  // Ratio between DSP and SSP (20:80) is set. Usually does not need to be changed
...
void Robot::calculate_swing_foot_piece_trajectory_heel_straight( int flag, double x_swing_foot_start, double x_swing_foot_end, double swing_foot_z_peak, double swing_foot_x_peak, double y,
    ...
    std::vector<double> t_posz(7), t_posx(5), t_theta(7), theta(7), x(5), z(7);
    // Calculate x
    double step_length = x_swing_foot_end-x_swing_foot_start;
...
//====================
    ...
    t_posz[0] = 0*cycle_duration;
    t_posz[1] = 0.01*cycle_duration;//cycle_duration - parameters.t_dsp;
    t_posz[2] = 0.1*cycle_duration;
    t_posz[3] = 0.5*cycle_duration;
    t_posz[4] = 0.9*cycle_duration;
    t_posz[5] = 0.99*cycle_duration;
    t_posz[6] = 1.0*cycle_duration;
    ...
    t_posx[0] = 0.00*cycle_duration;
    t_posx[1] = 0.01*cycle_duration;
    t_posx[2] = 0.5*cycle_duration;
    t_posx[3] = 0.99*cycle_duration;
    t_posx[4] = 1.00*cycle_duration;   
