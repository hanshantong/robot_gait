...
void KalmanFilter::setQ(double Q_proc){
    ...
    Q = Q_proc, Q_proc, Q_proc, Q_proc,
        Q_proc, Q_proc, Q_proc, Q_proc,
        Q_proc, Q_proc, Q_proc, Q_proc,
        Q_proc, Q_proc, Q_proc, Q_proc;
...
void KalmanFilter::initKalmanFilter(){
    ...
    //     0,0,1;
    A = 1, Ts, 0, 0,
        pow(omega,2)*Ts, 1, -pow(omega,2)*Ts, 0,
        0, 0, 0, 0,
        0,0,0,1;
    B.set_size(4,1);
    B = 0,0,1,0;
    ...
    P_aposteriori.set_size(4,4);
    P_aposteriori = dlib::identity_matrix<double>(3);

    x_aposteriori.set_size(4,1);
    x_aposteriori = 0,0,0,0;
