...
void Robot::set_parameters(){
    ...
    parameters.step_duration           = 2.5;     //* Duration for one step. This step consists of 80% SSP and 20% DSP. NOT gait cycle duration (which is 2*step_duration)
    ...
    parameters.zmp_offset_x            = 0;  // Offset in x direction for the ZMP. Use this variable to compensate the LIPM and MBM model error (ca 2-4 cm depending on walking speed)
    ...
    parameters.zmp_offset_y            = 0.033;  // Offset in x direction for the ZMP. Use this variable to compensate the LIPM and MBM model error (ca 2-4 cm depending on walking speed)
    ...
    parameters.zmp_offset_y_delta       = 0.003;
...
void Robot::calc_p_ref_transition_side(int leftside, int leftFootStart, int step_amount, double step_width){
    ...
    for (int i = 0; i < step_amount; i++){
        ...
        if(i == 0){
            ...
            zmp_offset_y = parameters.zmp_offset_y * 1.01 + parameters.zmp_offset_y_delta ;
    ...
    std::vector<double> * theta_ref_swing_foot_complete, std::vector<double> * psi_ref_swing_foot_complete)
{
    ...
    std::vector<double> t_posz(9), t_posy(14),t_posx(14), t_theta(7), theta(7), x(14), y(14), z(9);
...
bool Robot::kick_generator(int kickType, kickPoint input){
    ...
    //lift_buffer
    {
        ...
        {   
            ...
            end.z = parameters.robot_ankle_to_foot + 0.025;
            end.pitch = 0;
            end.yaw = 0;
            kick_swing_generator(end, time, paraSwing, pointNum);
        }
