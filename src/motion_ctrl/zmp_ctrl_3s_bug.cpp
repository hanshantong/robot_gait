...
void Robot::set_parameters(){
    ...
    parameters.Tipping_Scale_left      = 0.8; //88     //* 0.7-1 This value modifies the hip roll in case of soft joints. Due to control schemes, this value can be kept at 1. 
    ...
    parameters.Tipping_Scale_ankle_left      = 1; //88     //* 0.7-1 This value modifies the hip roll in case of soft joints. Due to control schemes, this value can be kept at 1. 
    parameters.Tipping_Scale_ankle_right     = 1; //88    //* This value modifies the hip roll in case of soft joints. Due to control schemes, this value can be kept at 1. 
...
void Robot::calc_p_ref_heel_side(int leftside, int leftFootStart, int step_amount/*Useless*/, double step_width){
    ...
    for (int i = 0; i < step_amount; i++){
        if(i > 2)
        {
            sy[i] = (i - floor(i / 2))*step_width - step_width * 0.5;
            sx[i] = 0;            
        }
        else
        {
...
void Robot::calc_p_ref_transition_side(int leftside, int leftFootStart, int step_amount, double step_width){
    ...
    for (int i = 0; i < step_amount; i++){
        ...
        if(i == 0){
            ...
            zmp_offset_y = parameters.zmp_offset_y * 1.03 + parameters.zmp_offset_y_delta ;
     ...
     // generate_orientated_spline(x,y,psi,samples, &xSpline, &ySpline);
        ...
        int Robot::walk(dxl_Actuator *dxl_actuator, float gait_packet[]){
           ...
           //  std::cout << "i: " << time_step << std::endl;
            ...
            //printf("LEFT is Support : %d\n",leftIsSupportLeg);
                    // Send the joint angles to the actuator
            dxl_actuator[0].set_goal_theta(joint_angles(0)*180/pi);           
            if(y_ref.at(time_step) > 0)
            {
                dxl_actuator[1].set_goal_theta(joint_angles(1)*180/pi*parameters.Tipping_Scale_left);           
            }else
            {
            ...
            dxl_actuator[3].set_goal_theta(joint_angles(3)*180/pi); 
            dxl_actuator[4].set_goal_theta(joint_angles(4)*180/pi);   
            dxl_actuator[5].set_goal_theta(joint_angles(5)*180/pi * parameters.Tipping_Scale_ankle_left);  
            ...
            dxl_actuator[9].set_goal_theta(joint_angles(9)*180/pi); 
            dxl_actuator[10].set_goal_theta(joint_angles(10)*180/pi);    
            dxl_actuator[11].set_goal_theta(joint_angles(11)*180/pi * parameters.Tipping_Scale_ankle_right);  
                    // Update time step
            time_step++;
